...
// 需要拦截的链接比如 str	__NSCFString *	@"/css/ntplugin/base.css"
+ (BOOL)canInitWithRequest:(NSURLRequest *)request
{
    if ([request.URL.scheme isEqualToString:@"http"]) {
        ...
        for (NSString *typeStr in sourceArr) {
            NSString *Regex = [HybridPreLoading addLocalFileReturnRex:typeStr];//遍历文件夹并把里面的文件名拼接成字符串
            if ([HybridPreLoading rexString:str rex:Regex success:nil faild:nil] && ![NSURLProtocol propertyForKey:hasInitKey inRequest:request]) { // 匹配里面是否有需要拦截的文件。比如现在str里有base.css，那么就可以拦截
                return YES;
            }
        }
