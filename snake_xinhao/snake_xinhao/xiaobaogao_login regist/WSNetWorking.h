 ...
 *
 */
typedef void (^WSDownloadProgress)(int64_t bytesRead,
                                   int64_t totalBytesRead);
typedef WSDownloadProgress WSGetProgress;
typedef WSDownloadProgress WSPostProgress;
 ...
 *
 */
typedef void (^WSUploadProgress)(int64_t bytesWritten,
                                 int64_t totalBytesWritten);


typedef NS_ENUM(NSUInteger,WSResponseType){

    kWSResponseTypeJSON = 1, //默认
    kWSResponseTypeXML = 2,  //XML
    // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
    kWSResponseTypeData = 3

};

typedef  NS_ENUM(NSUInteger,WSRequestType){

    kWSRequestTypeJSON = 1,      //默认
    kWSRequestTypePlainText = 2  //普通 text/html
...
// 且处理，请转换成对应的子类类型
typedef NSURLSessionDataTask WSURLSessionTask;
 ...
 *
 */
typedef void (^WSResponseSuccess)(id response);
 ...
 *
 */
typedef void (^WSResponseFail)(NSError * error);
