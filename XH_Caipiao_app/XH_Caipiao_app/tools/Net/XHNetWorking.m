...
#define VersionOnitunUrl @""
#ifdef DEBUG
#define XHAppLog(s, ... ) NSLog( @"[%@：in line: %d]-->%@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define XHAppLog(s, ... )
#endif

//static NSString *sg_privateNetworkBaseUrl = QDWBASE_URL;
static BOOL sg_isEnableInterfaceDebug = NO;
static BOOL sg_shouldAutoEncode = NO;
static NSDictionary *sg_httpHeaders = nil;
static XHResponseType sg_responseType = kXHResponseTypeJSON;
static XHRequestType  sg_requestType  = kXHRequestTypeJSON;
...
+ (XHURLSessionTask *)_requestWithUrl:(NSString *)url
    ...
    if ([self baseUrl] == nil) {
        if ([NSURL URLWithString:url] == nil ) {
            XHAppLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
            return nil;
        }
    }
        else{
...
+ (XHURLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    if ([NSURL URLWithString:uploadingFile] == nil) {
        XHAppLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
        return nil;
    }
    ...
    if (uploadURL == nil) {
        XHAppLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
        return nil;
    }
    ...
    XHURLSessionTask *session = [manager uploadTaskWithRequest:request fromFile:[NSURL URLWithString:uploadingFile] progress:^(NSProgress * _Nonnull uploadProgress) {
        if (progress) {
...
+ (XHURLSessionTask *)uploadWithImage:(UIImage *)image
    ...
    AFHTTPSessionManager *manager = [self manager];
    XHURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (XHURLSessionTask *)downloadWithUrl:(NSString *)url
                           saveToPath:(NSString *)saveToPath
                             progress:(XHDownloadProgress)progressBlock
                              success:(XHResponseSuccess)success
                                 fail:(XHResponseFail)fail{
    ...
    XHURLSessionTask *session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        if (progressBlock) {
...
+ (AFHTTPSessionManager *)manager {
    ...
    switch (sg_requestType) {
        case kXHRequestTypeJSON: {
            manager.requestSerializer = [AFJSONRequestSerializer serializer];
            [manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Accept"];
            [manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
            break;
        }
        case kXHRequestTypePlainText: {
    ...
    switch (sg_responseType) {
        case kXHResponseTypeJSON: {
            manager.responseSerializer = [AFJSONResponseSerializer serializer];
            break;
        }
        case kXHResponseTypeData: {
            manager.responseSerializer = [AFXMLParserResponseSerializer serializer];
            break;
        }
        case kXHResponseTypeXML: {
...
+ (void)successResponse:(id)responseData callback:(XHResponseSuccess)success {
    if (success) {
